...
If ((isset($_POST["numer_kolejny_zapisu"])) and ( !(empty($_POST["numer_kolejny_zapisu"]))))
    $numer_kolejny_zapisu = $_POST["numer_kolejny_zapisu"];
else
    $numer_kolejny_zapisu = "";
If ((isset($_POST["data_rejestracji_zapisu"])) and ( !(empty($_POST["data_rejestracji_zapisu"]))))
    $data_rejestracji_zapisu = $_POST["data_rejestracji_zapisu"];
else
    $data_rejestracji_zapisu = "";
If ((isset($_POST["data_rejestracji_dokumentu"])) and ( !(empty($_POST["data_rejestracji_dokumentu"]))))
    $data_rejestracji_dokumentu = $_POST["data_rejestracji_dokumentu"];
else
    $data_rejestracji_dokumentu = date("Y-m-d");
If ((isset($_POST["typ_umowy"])) and ( !(empty($_POST["typ_umowy"]))))
    $typ_umowy = $_POST["typ_umowy"];
else
    $typ_umowy = "";
If ((isset($_POST["rok_dokumentu"])) and ( !(empty($_POST["rok_dokumentu"]))))
    $rok_dokumentu = $_POST["rok_dokumentu"];
else
    $rok_dokumentu = "";
If ((isset($_POST["komu_wydany_stanowisko"])) and ( !(empty($_POST["komu_wydany_stanowisko"]))))
    $komu_wydany_stanowisko = $_POST["komu_wydany_stanowisko"];
else
    $komu_wydany_stanowisko = "";
If ((isset($_POST["nazwa_nadawcy_adresata"])) and ( !(empty($_POST["nazwa_nadawcy_adresata"]))))
    $nazwa_nadawcy_adresata = $_POST["nazwa_nadawcy_adresata"];
else
    $nazwa_nadawcy_adresata = "";
If ((isset($_POST["nazwa_dokumentu_lub_czego_dotyczy"])) and ( !(empty($_POST["nazwa_dokumentu_lub_czego_dotyczy"]))))
    $nazwa_dokumentu_lub_czego_dotyczy = $_POST["nazwa_dokumentu_lub_czego_dotyczy"];
else
    $nazwa_dokumentu_lub_czego_dotyczy = "";
If ((isset($_POST["numer_dokumentu_otrzymania"])) and ( !(empty($_POST["numer_dokumentu_otrzymania"]))))
    $numer_dokumentu_otrzymania = $_POST["numer_dokumentu_otrzymania"];
else
    $numer_dokumentu_otrzymania = "";
If ((isset($_POST["adnotacje"])) and ( !(empty($_POST["adnotacje"]))))
    $adnotacje = $_POST["adnotacje"];
else
    $adnotacje = "";
If ((isset($_POST["plik"])) and ( !(empty($_POST["plik"]))))
    $plik = $_POST["plik"];
else
    $plik = "";
If ((isset($_POST["komu_wydany_imie_i_nazwisko"])) and ( !(empty($_POST["komu_wydany_imie_i_nazwisko"]))))
    $komu_wydany_imie_i_nazwisko = $_POST["komu_wydany_imie_i_nazwisko"];
else
    $komu_wydany_imie_i_nazwisko = "";
If ((isset($_POST["id_umowy"])) and ( !(empty($_POST["id_umowy"]))))
    $id_umowy = $_POST["id_umowy"];
else
    $id_umowy = "";
If ((isset($_POST["wgrany_plik"])) and ( !(empty($_POST["wgrany_plik"]))))
    $wgrany_plik = $_POST["wgrany_plik"];
else
    $wgrany_plik = "";
If ((isset($_POST["id_zapisu"])) and ( !(empty($_POST["id_zapisu"]))))
    $id_zapisu = $_POST["id_zapisu"];
else
    $id_zapisu = "";
If ((isset($_POST["pdf1"])) and ( !(empty($_POST["pdf1"]))))
    $pdf1 = $_POST["pdf1"];
else
    $pdf1 = "";
If ((isset($_POST["uwagi"])) and ( !(empty($_POST["uwagi"]))))
    $uwagi = $_POST["uwagi"];
else
    $uwagi = "";
If ((isset($_POST["usun"])) and ( !(empty($_POST["usun"]))))
    $usun = $_POST["usun"];
else
    $wgrany_plik = $pdf1;
...
if (!empty(Session::get("uzytkownik")) AND Session::get("uzytkownik") > 0 and Session::get("Nazwa_bazy") == "Pieczecie") {
    ...
    if (isset($_GET["kat"]) AND ( $_GET["kat"] == 10)) {
        ...
        mysql_close();
        ?>
                                                <form method="post" action="pieczecie.php" enctype="multipart/form-data">
                                                    <table width="100%" cellspacing="0" cellpadding="0" align="center" border="0">
                                                        ...
                                                        <tr height="50">
                                                            ...
                                                            <td align="left" width="450"><input value="<?php echo $data_rejestracji_dokumentu; ?>" type="text" name="data_rejestracji_dokumentu" size="60" maxlength="10" class="inputbox"></td>
                                                        </tr>
                                                        <tr height="50">     
                                                        ...
                                                        <tr height="50">
                                                            <td align="left" width="250">Adnotacje o wysłaniu dokumentu, przekazaniu do pieczecie lub nr pozycji teczki tematycznej, w której dokument si﻿ę znajduje:</td>
    ...
    if (isset($_POST["kat"]) AND ( $_POST["kat"] == 11)) {
        ...
        if (($numer_kolejny_zapisu <> '') AND ( $row == 0)) {
            ...
            $result = mysql_query($query);
            echo ("Dokument dodano!");
        } else {
            if (($numer_kolejny_zapisu <> '') AND ( $row > 0)) {
                echo ("<script language=\"JavaScript\">alert('Dokument istnieje!');</script>");
            } else {
                echo ("<script language=\"JavaScript\">alert('Nie podałeś numeru kolejnego zapisu!');</script>");
            }
        }
        mysql_close();
    }
    ...
    if (isset($_GET["kat"]) AND ( $_GET["kat"] == 20)) {
        ?>
                                                <form method="post" action="pieczecie.php">
                                                    <table width="100%" cellspacing="0" cellpadding="0" align="center" border="0">
                                                        <tr>
                                                            <td colspan="2">&nbsp;</td>
                                                        </tr>
...
#	 $numer_kolejny_zapisu = preg_replace('/\s+/', '', $numer_kolejny_zapisu1);
        ...
        if ($row['skan'] != '') {
            ...
            $pole1 = "Usuń skan w formacie PDF <input type=\"checkbox\" name=\"usun\" vaule=\"tak\" /><input type=\"hidden\" value=\"$skan\" name=\"pdf1\" />";
    ...
    if (isset($_POST["kat"]) AND ( $_POST["kat"] == 22)) {
        ...
        if (sizeof($_FILES) != 0 and $_FILES['plik']['type'] == "application/pdf") {
            $wgrany_plik = md5($_FILES['plik']['name'] . time() . mt_rand()) . ".pdf";
            $kat_pliki = constant('KATALOG_PIECZATKI');
            $kat_pliki = $kat_pliki . basename($wgrany_plik);
            $przeniesienie = move_uploaded_file($_FILES['plik']['tmp_name'], $kat_pliki);
        }
#	 if ($_GET('usun')==on) {
#	 print "ON";
#	 }
#	int "$usun UPDATE rejestr_umow SET data_rejestracji_zapisu=\"$data_rejestracji_dokumentu\", nazwa_nadawcy_adresata=\"$nazwa_nadawcy_adresata\", numer_dokumentu_otrzymania=\"$numer_dokumentu_otrzymania\", nazwa_dokumentu_lub_czego_dotyczy=\"$nazwa_dokumentu_lub_czego_dotyczy\", uzytkownik_id=\"$uzytkownik\", kategoria=\"$typ_umowy\", skan=\"$pdf1\" WHERE id=$id_zapisu";
                                                ...
                                                <table width="100%" cellspacing="0" cellpadding="0" align="center" border="0">
                                                    <tr height="50" bgcolor="#f4f4f4">
                                                        ...
                                                        for ($i = 0; $i < $row; $i++):
                                                            ...
                                                            echo ("
			    <tr height=\"40\" onclick=\"Startgrafika('../pieczecie/dokument.php?id=$id')\" style=\"cursor: hand\">
    ...
    if (isset($query1))
        $queryResult = $queryResult . " " . $query1;
    if (isset($query2))
